...
void DMMOTOR::DMmotor_transmit()
{
    ...
    if (idx >= 6) return;                 // �?jointpdata[6][8]，留个保�?

    // 先把共享�?字节拷贝到局部，避免发送时被更新任务改�?
    uint8_t txbuf[8];
...
void DMMOTOR::DMmotorinit()
{
    ...
    for (uint32_t i = 0; i < 3; ++i) {          // 使能�?个ID电机
...
void DMMOTOR::CanComm_ControlCmd(CAN hcan, uint8_t cmd, uint32_t id)//使能�?
{
